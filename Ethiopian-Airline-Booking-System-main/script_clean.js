...
function renderSeatMap(flightNumber) {
    ...
    for (let r = rowStart; r <= rowEnd; r++) {
        for (let c = 0; c < cols.length; c++) {
            ...
            // Mark reserved if occupied or locked by another user
            if (occupied.includes(seatId)) {
                ...
                // check seat lock
                const locked = isSeatLocked(flightNumber, seatId);
                if (locked) {
                    const locks = lsGet('seatlocks', {});
                    const lock = locks[flightNumber] && locks[flightNumber][seatId];
                    const owner = lock ? lock.username : null;
                    // If lock belongs to current user, allow selection; otherwise mark reserved
                    if (!currentUser || owner !== currentUser.username) {
                        el.classList.remove('available');
                        el.classList.add('reserved');
                        el.setAttribute('aria-disabled', 'true');
                    }
